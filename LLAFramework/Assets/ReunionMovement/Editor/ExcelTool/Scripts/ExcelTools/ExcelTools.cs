    ...
    public class ExcelTools
    {
        ...
        static List<SheetData> ExcelToScripts(string path, bool createScriptableObjects = false)
        {
            ...
            foreach (DataTable table in excel.ResultSet.Tables)
            {
                ...
                if (!FileOperation.CheckClassName(sheet.itemClassName))
                {
                    EditorUtility.ClearProgressBar();
                ...
                for (int i = 0, imax = fieldNames.Length; i < imax; i++)
                {
                    ...
                    if (!FileOperation.CheckFieldName(fieldNameStr))
                    {
                        EditorUtility.ClearProgressBar();
...
";
            ...
            await FileOperation.SaveFile(scriptOutPutPath + dataName + ".cs", str);
...
";
            ...
            await FileOperation.SaveFile(scriptOutPutPath + dataName + "DTO.cs", str);
...
";
            ...
            await FileOperation.SaveFile(scriptOutPutPath + dataName + "Container.cs", str);
...
";
        ...
        ");
            ...
            str = str.Replace("{_CREATE_TIME_}", DateTime.UtcNow.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"));
            await FileOperation.SaveFile(scriptOutPutPath + "SqliteMgr.cs", str);

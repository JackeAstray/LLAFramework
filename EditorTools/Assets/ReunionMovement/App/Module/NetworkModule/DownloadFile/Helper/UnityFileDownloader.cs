    ...
    public class UnityFileDownloader : IDownloader
    {
        ...
        public float BytesDownloadedPerSecond
        {
            get
            {
                float tb = 0;
                float elapsed = 0;
                foreach (var idf in fulfillersOld)
                {
                    tb += idf.BytesDownloaded;
                    elapsed += idf.ElapsedTime;
                }
                foreach (var idf in fulfillers)
                {
                    tb += idf.BytesDownloaded;
                    elapsed += idf.ElapsedTime;
                }
        ...
        public override async Task<bool> Download(string uri)
        {
            ...
            if (fulfillers.ToList().Select(idf => idf.Uri == uri).ToList().Count == 1)
            {
                // case: queued (to be fulfilled)
            }
            else if (fulfillersOld.ToList().Select(idf => idf.Uri == uri).ToList().Count == 0)
            {
                ...
                fulfillers.Add(new IDownloadFulfiller[] { idf });
            ...
            else
            {
                ...
                IDownloadFulfiller idf = fulfillersOld.ToList().Where(idf => idf.Uri == uri).ToArray()[0];
                var v = fulfillersOld.ToList();
                v.Remove(idf);
                fulfillersOld = v.ToArray();
                _PendingUris.Add(new string[] { uri });
                fulfillers.Add(new IDownloadFulfiller[] { idf });
            }
        ...
        internal Task<bool> _Dispatch()
        {
            ...
            IDownloadFulfiller idf = fulfillers[0];
            _PendingUris = _PendingUris.Dequeue();
            fulfillers = fulfillers.Dequeue();
            fulfillersOld = fulfillersOld.Add(new IDownloadFulfiller[] { idf }); // add to old
            ...
            UnityWebRequestAsyncOperation rv = idf.Download();
            if (rv == null)
            { // case: either multi-part completed or file is fully downloaded already
                _DispatchCompletion();
                return _ReturnFalseAsync();
            }
            _N++;
        ...
        {
            return fulfillers.Where(idf => idf.Uri == uri).ToArray().Length == 0 ? fulfillersOld.Where(idf => idf.Uri == uri).ToArray()[0] : fulfillers.Where(idf => idf.Uri == uri).ToArray()[0];
        }
        ...
        public override async Task<bool> Cancel(string uri)
        {
            ...
            {
                // case: queued (to be fulfilled)
                IDownloadFulfiller idf = fulfillers.ToList().Where(idf => idf.Uri == uri).ToArray()[0];
                idf.Cancel();
                var v = fulfillers.ToList();
                v.Remove(idf);
                fulfillers = v.ToArray();
                fulfillersOld = fulfillersOld.Add(new IDownloadFulfiller[] { idf });
            }
            else if (fulfillersOld.ToList().Select(idf => idf.Uri == uri).ToList().Count == 0)
            {
        ...
        {
            if (AbandonOnFailure) foreach (var idf in fulfillers)
                {
                    idf.Cancel();
                }
            if (AbandonOnFailure) foreach (var idf in fulfillersOld)
                {
                    idf.Cancel();
                }
        }
        ...
        public override void Reset()
        {
            ...
            incompletedUris = null;
            fulfillers = new IDownloadFulfiller[0];
            fulfillersOld = new IDownloadFulfiller[0];
            uris = new string[0];
        }
